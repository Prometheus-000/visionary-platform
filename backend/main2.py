...
#
# create_if_missing=True: on a fresh account/workspace this Volume doesn't
# exist yet. That's fine — it starts empty and self-populates, weights via
# _ensure_weight() on the first train/generate call, datasets via the upload
# endpoints — so auto-creating an empty one here is a normal starting state,
# not a workaround.
volume = modal.Volume.from_name("forge-webui-repo", create_if_missing=True)
...
#
# This is the one piece that genuinely can't be made account-portable in code:
# a Secret holds real credentials (your HF token, your chosen API key), and
# Modal deliberately has no create_if_missing for Secret — auto-creating a
# blank one would just move today's clean deploy-time failure to a confusing
# one on the first real request. Run the command above once per new
# account/workspace before `modal deploy`; _check_key() below already surfaces
# this same command if it's ever missing at request time.
