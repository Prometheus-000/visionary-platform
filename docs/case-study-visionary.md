...
**On Hopper and Blackwell:** the inference image is currently **Hopper-only** (`H100`/`H200`, `sm_90`) on purpose — SageAttention's kernels are compiled for one architecture, and `sm_80` is not `sm_90`, which is what moved Krea 2 off the A100 it used to run on. Blackwell (`B200`, `sm_100`) is a *deliberate one-line pathway*, not a silent capability: bump `TORCH_CUDA_ARCH_LIST` to `10.0` and force a rebuild, or the kernels load the model and then fall back to the slow path. `max_containers=1` per GPU class keeps exactly one warm checkpoint resident, so a second replica never pays a cold load to share a job it could have queued behind.
