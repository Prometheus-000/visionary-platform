...
Because the whole app scales to zero on Modal, there is no "keep alive," no idle bill, and no lifecycle the user has to manage. The cost of that trade is cold starts, and the project has now twice decided *not* to hide them by spending a GPU: a page-load ping warmed the interpreter's L4 for months after nothing routed there, and when it was repointed at the generator it became a real card — with a ten-minute scaledown window — rented by the act of opening a tab. **A GPU starts on Generate and at no other time**; the ~25 seconds of engine start land in front of a status line that says what it is doing, which is the wait this project is willing to spend. What still happens where the user isn't looking is the cheap half: a family of weights downloads on **CPU containers**, never on a rented GPU idling next to a 26 GB pull.
